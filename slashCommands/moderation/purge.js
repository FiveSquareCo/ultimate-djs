...
module.exports = {
    ...
    run: async (interaction, args) => {
        ...
        const messages = await interaction.channel.bulkDelete(
            noOfMessagesToDelete
        );
        let messagesLinkDesc = "";
        messages.map((message, index) => {
            messagesLinkDesc += `(${message.id}) ${new Date(
                message.createdTimestamp
            ).toLocaleDateString()} "${message.author.tag}"(${
                message.author.id
            }) -- ${message.content || "this was an embed message"} \n\n`;
        });
        const messagesLink = await createBin(
            messagesLinkDesc,
        ...
        const deletedMessagesEmbed = new MessageEmbed()
            .setColor(4437378)
            .setDescription(`Deleted ${noOfMessagesToDelete} messages!`);
        interaction.reply({
...
const purgeModlogs = (
    ...
    const purgelogsEmbed = new MessageEmbed()
        ...
        .setTimestamp()
        .setDescription(
            `${moderator.tag} deleted ${messageCount} in <#${channel.id}> on ${date}`
        )
        .setFields(
            {
                name: "Moderator",
