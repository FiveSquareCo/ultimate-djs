    ...
    run: async (interaction, args) => {
        ...
        if (!results || !results.warnings.length) {
            return errorMessageEmbed(
                interaction,
                "Please provide a valid warn id!",
                "SC"
            );
        }
        ...
        return successMessageEmbed(
            interaction,
            `Deleted warn (${results.warnings[0].id}) for **${
                interaction.client.users.cache.get(results.userId).tag
            }**`
        );
    },
};
...
const delWarnModLog = async (interaction, moderator, results) => {
    ...
    const wMod = interaction.client.users.cache.get(
        results.warnings[0].moderator
    );
    const date = `<t:${(new Date() / 1000).toFixed()}:R>`;
    const givenDate = `<t:${(
        new Date(results.warnings[0].timestamp) / 1000
    ).toFixed()}:R>`;
    const warnEmbed = new MessageEmbed()
        .setColor(3092790)
        .setTimestamp()
        .setDescription(
            `${moderator.tag} deleted 1 warn for ${user.tag} on ${date}`
        )
        .addField("Moderator", `${moderator.tag} - ${moderator.id}`, true)
        .addField("Member", `${user.tag} - ${user.id}`, false)
        .addField(
            "Warning",
            `> **Id :** ${results.warnings[0].id}\n> **Given by ** ${wMod.tag} - ${wMod.id}\n> **Given on :** ${givenDate}\n> **Reason :** ${results.warnings[0].reason}`
        )
        .setAuthor("Mod Logs");
