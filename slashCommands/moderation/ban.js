...
module.exports = {
    ...
    run: async (interaction, args) => {
        ...
        if (
            member.member.roles.highest.position >=
            moderator.roles.highest.position
        ) {
            return errorMessageEmbed(
                interaction,
                "Cannot ban the user | **Reason :** the member's role is highr than yours"
            );
        }
        if (!member.member.bannable) {
            return errorMessageEmbed(
                interaction,
                "I cannot ban users with role higher than me!"
            );
        }
        ...
        successMessageEmbed(
            interaction,
            `Banned **${member.user.tag}** from server | **Reason :** ${reason}`
        );
    },
};
...
const banModlogGen = (interaction, moderator, member, reason) => {
    ...
    const banLogsEmbed = new MessageEmbed()
        .setAuthor("Mod Logs")
        .setDescription(
            `${moderator.user.tag} banned ${member.user.tag} on ${banDate}`
        )
        .setColor(3092790)
        .setTimestamp()
